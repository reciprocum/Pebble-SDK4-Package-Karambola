...
)
{
      ...
      if (this->normal_worldCoord)
        ...
        { // #1: Assume/mark all mesh edges as invisible.
          ...
          for ( uint8_t mf = 0 ; mf < facesNum ; ++mf )
          {
            ...
            const Q3 *anchorOfFaceNormal = &vertices[edges[f->faceInfo->edgeIndexes[0]].v1].worldCoord ;
  ...
  for ( unsigned int me = 0  ;  me < edgesNum  ;  ++me )
    if (!edgesState[me].isDisabled)  // Skip disabled edges.
    {
      ...
      const GPoint p0 = __MeshQ3_vertex_screenPoint[edges[me].v1] ;
      const GPoint p1 = __MeshQ3_vertex_screenPoint[edges[me].v2] ;
